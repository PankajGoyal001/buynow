          ...
          <div className="sm:flex sm:gap-6 items-center grid gap-[10px] ">
            {
              ProcessData.map((items, index) => (
                <div key={index} className='flex md:max-w-[270px] h-[190px] hover:cursor-pointer p-5 hover:shadow-xl rounded-[20px] border w-full flex-col gap-5'>
                  <img className='max-w-10 w-full' src={`/src/assets/svg/${items.Images}`} alt="image" />
                  <div className="flex flex-col gap-[10px]">
                    <p className="text-xl leading-[123.5%] font-heebo_Medium tracking-[0.15px] text-gray-graystrong">{items.Title}</p>
